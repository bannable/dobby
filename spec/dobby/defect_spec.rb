...
RSpec.describe Dobby::Defect do
  ...
  let(:unfixed_package) { Fixed.new(Dobby::Package::MAX_VERSION) }
  ...
  let(:defect) { Dobby::Defect.new(defect_hash) }
    ...
    context 'when not flag filtered' do
      ...
      context 'filter is not recognized' do
        ...
        it 'raises an UnknownFilterError' do
          expect { subject }.to raise_error(Dobby::UnknownFilterError, 'foo')
        end
      end
    end
  end
