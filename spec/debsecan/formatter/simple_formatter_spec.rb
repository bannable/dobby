...
RSpec.describe Debsecan::Formatter::SimpleFormatter do
  subject(:formatter) { described_class.new(output) }

  let(:output) { StringIO.new }

  let(:package) { instance_double('Debsecan::Package', to_s: 'PKG') }

  let(:low_defect) do
    instance_double('Debsecan::Defect',
                    severity: Debsecan::Severity::Low,
                    identifier: 'FAKE-LOW')
  end

  let(:medium_defect) do
    instance_double('Debsecan::Defect',
                    severity: Debsecan::Severity::Medium,
                    identifier: 'FAKE-MEDIUM')
  end

  let(:critical_defect) do
    instance_double('Debsecan::Defect',
                    severity: Debsecan::Severity::Critical,
                    identifier: 'FAKE-CRITICAL')
  end

  let(:unknown_defect) do
    instance_double('Debsecan::Defect',
                    severity: Debsecan::Severity::Unknown,
